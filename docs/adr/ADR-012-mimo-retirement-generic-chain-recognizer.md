...
**B-v1（本轮）**：线性链通用捕获，范围 = `{ReLU, Add, MatMul}` 白名单、firing 节点单输入、链必含 MatMul（层边界）、链长 ≤5。`C3_MIMO_GENERIC=1` 启用，位于手写识别器**之前**（miss 即透传），默认关闭 → 默认行为零变化。

**B-v2（后续）**：树拓扑（FFN SwiGLU）通用捕获 + Tanh/Sigmoid 纳入白名单（依赖 tanh 反向图执行层专项）+ 默认切换。

### 关键设计决策（B-v1）

1. **恒等子图 → 别名链接**：Add 输入形状 == grad 形状时其梯度是恒等透传。历史 worker 对「无算力节点」图有跳过 bug（`C3BackwardCapture.cpp:243`），故**不建恒等子图**，改为把下游链接直接指向上游的 grad 输出（与 legacy FC 的「grad_z 直连 mm_w/mm_x/add_b」结构完全一致），执行侧按别名把 pending 映射到既有输出槽。
2. **编译线程只传值**：链结构（类型序列 + grad desc + 各输入 desc + 边输入索引）同步提取、按值传编译线程，不引用 Node*（沿用 MIMO pending 生命周期模式）。
